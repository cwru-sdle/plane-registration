...
for d in "$BATCH_DIR"/*; do
    if [[ -d "$d" ]]; then
        ...
        # Create tar and transfer via SSH with ProxyCommand instead of -J
        if tar -cf - -C "$d" . | ssh -o "ProxyCommand ssh $REMOTE_USER@$JUMP_HOST -W $REMOTE_HOST:22" "$REMOTE_USER@$REMOTE_HOST" bash -c "'
