...
REMOTE_USER="aml334"
JUMP_HOST="pioneer.case.edu"
REMOTE_HOST="cradle3202t.priv.cwru.edu"
...
for d in "$BATCH_DIR"/*; do
    if [[ -d "$d" ]]; then
        ...
        # Check if batch directory has files
        if ! ls "$d"/* >/dev/null 2>&1; then
            echo "Warning: $batch_name is empty, skipping"
            continue
        fi
        
        # Debug: Print the SSH command that will be executed
        echo "Debug: SSH command will be:"
        echo "ssh -J $REMOTE_USER@$JUMP_HOST $REMOTE_USER@$REMOTE_HOST"
        
        # Create tar and transfer via SSH
        if tar -cf - -C "$d" . | ssh -J "$REMOTE_USER@$JUMP_HOST" "$REMOTE_USER@$REMOTE_HOST" "
            kinit -kt /home/aml334/aml334.keytab aml334@ADS.CASE.EDU && \
            hdfs dfs -put - /user/$REMOTE_USER/${batch_name}.tar
        "; then
