...
def analyze_group_autocorrelation(df, group_col='group_id', sensor_cols=['sensor0', 'sensor1'], 
    ...
    for group in groups:
        ...
        # For very large datasets, use sampling or chunking
        if n_points > sample_size:
            print(f"  Large dataset detected. Using sampling and chunking approach...")
            
            # Strategy 1: Random sampling for quick overview
            sample_indices = np.random.choice(n_points, sample_size, replace=False)
            sample_indices = np.sort(sample_indices)  # Keep temporal order
            sampled_data = group_data[sample_indices]
            
            # Strategy 2: Also analyze chunks for robustness
            chunk_size = sample_size // 10  # 10k points per chunk
            n_chunks = min(10, n_points // chunk_size)  # Max 10 chunks
        ...
        for sensor in sensor_cols:
            if sensor in group_data.columns:
                ...
                if len(autocorr) > 1:
                    ...
                    # For large datasets, also do chunk analysis
                    if n_points > sample_size and n_chunks > 1:
                        chunk_autocorrs = []
                        for i in range(n_chunks):
                            start_idx = i * (n_points // n_chunks)
                            end_idx = (i + 1) * (n_points // n_chunks)
                            chunk_data = group_data[start_idx:end_idx].select(sensor).to_numpy().flatten()
                            
                            if len(chunk_data) > 100:  # Minimum chunk size
                                _, chunk_autocorr = compute_autocorrelation_fast(chunk_data, min(max_lag, 20))
                                if len(chunk_autocorr) > 1:
                                    chunk_autocorrs.append(chunk_autocorr[1])  # Lag-1 autocorr
                        
                        if chunk_autocorrs:
                            group_result[f'{sensor}_chunk_mean_autocorr'] = np.mean(chunk_autocorrs)
                            group_result[f'{sensor}_chunk_std_autocorr'] = np.std(chunk_autocorrs)
                            group_result[f'{sensor}_chunk_consistency'] = 1 - (np.std(chunk_autocorrs) / (np.mean(np.abs(chunk_autocorrs)) + 1e-8))
...
def run_autocorrelation_analysis(parquet_path, output_dir=None):
    ...
    results = analyze_group_autocorrelation(df, max_lag=50, sample_size=100000)  # Increased sample size
